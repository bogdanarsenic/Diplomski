...
import { componentHookMethodNames, } from './component-hook-method-names.type';
export class ComponentHooksProbe extends Probe {
    doInitialize(ngZone, ngModule) {
        this.ngModule = ngModule;
        const probe = this;
        const componentTypes = getAllObjectsInAngularApplication(getComponentsFromModule, this.ngModule);
        componentTypes.forEach(componentType => {
            const probeHookMethod = (hookMethod) => {
                const original = componentType.prototype[hookMethod];
                componentType.prototype[hookMethod] = function (...parameters) {
                    const componentInstance = this;
                    probe.emit(() => new ComponentLifecycleMethodInvokedEvent(hookMethod, componentType, componentInstance, parameters));
                    if (original) {
...
//# sourceMappingURL=component-hooks-probe.class.js.map